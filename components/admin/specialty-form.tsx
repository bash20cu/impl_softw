...
import { createSpecialtyAction, type CatalogFormState, updateSpecialtyAction } from "@/lib/catalog-actions";
...
function SubmitButton({ mode }: { mode: "create" | "edit" }) {
  ...
  return (
    <Button className="rounded-full px-6" disabled={pending} type="submit">
      ...
      {pending ? "Guardando..." : mode === "create" ? "Crear especialidad" : "Guardar cambios"}
    </Button>
  );
}
...
export function SpecialtyForm({
  ...
  const action = mode === "create"
    ...
    : updateSpecialtyAction.bind(null, specialtyId ?? "");

  const [state, formAction] = useActionState<CatalogFormState, FormData>(action, initialState);

  return (
    <form action={formAction} className="space-y-6">
      <Badge variant="secondary">{mode === "create" ? "Nueva especialidad" : "Editar especialidad"}</Badge>
